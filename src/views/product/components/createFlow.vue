    ...
    <div class="home">
        <el-form ref="form" :model="form" label-width="0" :rules="rules" style="width: 100%;height: 100%">
            ...
            <GridLayout
                    ...
                    :layout="nodes">
                <svg xmlns="http://www.w3.org/2000/svg" version="1.1" id="svg-container" class="svg-container"
                     @click.self="cancleEvent" @mousemove="mouseMove">
                    <g @click="removeLine(key)" v-for="(item,key,index) in relations" class="svf-g">
                        <path
                                ...
                                fill="none"
                                class="paths"/>
                        <image v-if="item.bezier.endY" xlink:href="@/assets/right.png" width="30" height="30"
                               :x="item.bezier.endX-10" :y="item.bezier.endY-15"></image>
                    </g>
                    <g v-for="(item,index) in todoLine">
                        <path
                                ...
                                class="paths"/>
                        <image v-if="item.bezier.endY" xlink:href="@/assets/right.png" width="30" height="30"
                               :x="item.bezier.endX-10" :y="item.bezier.endY-15" @click.self="cancleEvent"></image>
                ...
                <grid-item v-for="(item,index) in form.nodes"
                    ...
                    <div ref="itemSlot"
                        ...
                        <i v-if="index==0">
                            <el-select clearable v-model="item.node_suppliers[0].supplier"
                                       placeholder="Select Specifications..." class="firstGroup" @change="rootChange">
                                <el-option
                                ...
                                <div class="flow-add-group-item" v-for="(group,gIndex) in item.node_suppliers">
                                    ...
                                    <div class="flow-form-item-right">
                                        <i class="el-icon-error"
                                           @click="removeFormItem(item.node_suppliers,gIndex)"></i>
                                    </div>
                                </div>
                            </div>
                    ...
                    <span class=" transition-box" v-show="!item.active && item.x!=tabs.length-1">
                           <i class="el-icon-circle-plus-outline el-icon-circle-plus operationIcon"
                              :class="[index==0?'isFirstPlus':'']" @click.stop="addItem(index)"></i>
                    </span>
                        </transition>
                        <transition name="el-zoom-in-center">
                    <span class=" transition-box" v-show="item.active">
                          <i class="el-icon-remove icon-remove operationIcon" :class="[index==0?'isFirstPlus':'']"
                             @click.stop="removeItem(item.relate,item.parentRelate,item.i,item.parentId)"></i>
                    </span>
                        </transition>
                        <template v-if="index!=0 ">
                            <!--                    :class="[item.active?'active-icon':'']"-->
                            <i class="el-icon-s-help icon-up position-icon" v-show="item.active|| connectActive"
                               @click="mouseDrag(item,'up')"></i>
                            <i class="el-icon-s-help icon-down position-icon" v-show="item.active|| connectActive"
                               @click="mouseDrag(item,'down')"></i>
                            <i class="el-icon-s-help icon-left position-icon" v-show="item.active || connectActive "
                               @click="mouseDrag(item,'left')"></i>
                        </template>
                        <i v-if="index!=0 " class="el-icon-s-help icon-right position-icon"
                           @click="mouseDrag(item,'right')"></i>
