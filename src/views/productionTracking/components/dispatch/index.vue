<template>
    <MyContainer>
        <MyAction @on-reset="onReset" @on-search="onSearch">
            <MyButton :btn="'not'" title="Dispatch" :disabled="multiple.length!=1" @click="hadnleInfo('hold')"></MyButton>
...
<script>
    ...
    export default {
        name: "productionTrackingDispatch",
        ...
        setup(props, ctx) {
            ...
            const {
                ...
                apiResponse,
                pageChange
            } = useList(ctx,'productionTracking/dispatch');
